...
MODELING_DIR = CURRENT_DIR.parents[1] / "4.modeling" / "classic_ML_model" / "4.Consumer Defensive"
...
def get_best_entry(model_comparison: dict) -> dict:
    ...
    if not eligible_models:
        raise ValueError("No eligible models found for Consumer Defensive.")
...
def print_best_model_summary(bundle: dict):
    ...
    print(f"Best Consumer Defensive model: {best_model_entry['model']}")
...
def main():
    global best_model_CD
    bundle = get_best_model_bundle()
    best_model_CD = bundle["best_model"]
