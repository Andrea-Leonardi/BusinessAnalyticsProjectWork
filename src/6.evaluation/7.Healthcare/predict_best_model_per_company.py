...
def predict_with_model(model_bundle: dict, X_test: pd.DataFrame) -> np.ndarray:
    ...
    if not selected_variables:
        raise ValueError("Selected variables file not found or empty for Healthcare.")
...
def build_prediction_comparison_dataframe() -> pd.DataFrame:
    ...
    healthcare_test_df, X_test_healthcare = build_sector_test_data(HEALTHCARE_SECTOR_CODE)
    y_pred_best_model = predict_with_model(model_bundle, X_test_healthcare)

    comparison_df = healthcare_test_df[["Ticker", DATE_COL, TARGET_COL]].copy()
