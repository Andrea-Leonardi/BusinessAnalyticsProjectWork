...
CONSUMER_CYCLICAL_SECTOR_CODE = 3
...
def predict_with_model(model_bundle: dict, X_test: pd.DataFrame) -> np.ndarray:
    ...
    if not selected_variables:
        raise ValueError("Selected variables file not found or empty for Consumer Cyclical.")
...
def build_prediction_comparison_dataframe() -> pd.DataFrame:
    ...
    consumer_cyclical_test_df, X_test_consumer_cyclical = build_sector_test_data(
        CONSUMER_CYCLICAL_SECTOR_CODE
    )
    y_pred_best_model = predict_with_model(model_bundle, X_test_consumer_cyclical)

    comparison_df = consumer_cyclical_test_df[["Ticker", DATE_COL, TARGET_COL]].copy()
