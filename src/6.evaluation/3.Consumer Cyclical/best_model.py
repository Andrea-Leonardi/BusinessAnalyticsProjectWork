...
MODELING_DIR = CURRENT_DIR.parents[1] / "4.modeling" / "classic_ML_model" / "3.Consumer Cyclical"
...
def get_best_entry(model_comparison: dict) -> dict:
    ...
    if not eligible_models:
        raise ValueError("No eligible models found for Consumer Cyclical.")
...
def print_best_model_summary(bundle: dict):
    ...
    print(f"Best Consumer Cyclical model: {best_model_entry['model']}")
...
def main():
    global best_model_CC
    bundle = get_best_model_bundle()
    best_model_CC = bundle["best_model"]
