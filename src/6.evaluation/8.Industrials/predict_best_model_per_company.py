...
def predict_with_model(model_bundle: dict, X_test: pd.DataFrame) -> np.ndarray:
    ...
    if not selected_variables:
        raise ValueError("Selected variables file not found or empty for Industrials.")
...
def build_prediction_comparison_dataframe() -> pd.DataFrame:
    ...
    industrials_test_df, X_test_industrials = build_sector_test_data(INDUSTRIALS_SECTOR_CODE)
    y_pred_best_model = predict_with_model(model_bundle, X_test_industrials)

    comparison_df = industrials_test_df[["Ticker", DATE_COL, TARGET_COL]].copy()
