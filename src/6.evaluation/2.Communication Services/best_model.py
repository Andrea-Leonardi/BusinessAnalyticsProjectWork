...
MODELING_DIR = CURRENT_DIR.parents[1] / "4.modeling" / "classic_ML_model" / "2.Communication Services"
...
def get_best_entry(model_comparison: dict) -> dict:
    ...
    if not eligible_models:
        raise ValueError("No eligible models found for Communication Services.")
...
def print_best_model_summary(bundle: dict):
    ...
    print(f"Best Communication Services model: {best_model_entry['model']}")
...
def main():
    global best_model_CS
    bundle = get_best_model_bundle()
    best_model_CS = bundle["best_model"]
