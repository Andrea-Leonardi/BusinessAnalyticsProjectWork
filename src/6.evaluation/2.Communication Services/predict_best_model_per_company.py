...
def predict_with_model(model_bundle: dict, X_test: pd.DataFrame) -> np.ndarray:
    ...
    if not selected_variables:
        raise ValueError("Selected variables file not found or empty for Communication Services.")
...
def build_prediction_comparison_dataframe() -> pd.DataFrame:
    ...
    communication_services_test_df, X_test_communication_services = build_sector_test_data(
        COMMUNICATION_SERVICES_SECTOR_CODE
    )
    y_pred_best_model = predict_with_model(model_bundle, X_test_communication_services)

    comparison_df = communication_services_test_df[["Ticker", DATE_COL, TARGET_COL]].copy()
