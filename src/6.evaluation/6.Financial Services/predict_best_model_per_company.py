...
FINANCIAL_SERVICES_SECTOR_CODE = 6
...
def predict_with_model(model_bundle: dict, X_test: pd.DataFrame) -> np.ndarray:
    ...
    if not selected_variables:
        raise ValueError("Selected variables file not found or empty for Financial Services.")
...
def build_prediction_comparison_dataframe() -> pd.DataFrame:
    ...
    financial_services_test_df, X_test_financial_services = build_sector_test_data(
        FINANCIAL_SERVICES_SECTOR_CODE
    )
    y_pred_best_model = predict_with_model(model_bundle, X_test_financial_services)

    comparison_df = financial_services_test_df[["Ticker", DATE_COL, TARGET_COL]].copy()
