...
def predict_with_model(model_bundle: dict, X_test: pd.DataFrame) -> np.ndarray:
    ...
    if not selected_variables:
        raise ValueError("Selected variables file not found or empty for Basic Materials.")
...
def build_prediction_comparison_dataframe() -> pd.DataFrame:
    ...
    basic_materials_test_df, X_test_basic_materials = build_sector_test_data(
        BASIC_MATERIALS_SECTOR_CODE
    )
    y_pred_best_model = predict_with_model(model_bundle, X_test_basic_materials)

    comparison_df = basic_materials_test_df[["Ticker", DATE_COL, TARGET_COL]].copy()
