...
MODELING_DIR = CURRENT_DIR.parents[1] / "4.modeling" / "classic_ML_model" / "1.Basic Materials"
...
def get_best_entry(model_comparison: dict) -> dict:
    ...
    if not eligible_models:
        raise ValueError("No eligible models found for Basic Materials.")
...
def print_best_model_summary(bundle: dict):
    ...
    print(f"Best Basic Materials model: {best_model_entry['model']}")
...
def main():
    global best_model_BM
    bundle = get_best_model_bundle()
    best_model_BM = bundle["best_model"]
