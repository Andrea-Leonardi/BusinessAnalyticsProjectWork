...
def download_income_statements(
    ...
    """
    # config.py provides the final CSV path, so create only its parent folder.
    output_file.parent.mkdir(parents=True, exist_ok=True)

    # Recover from an older bug where the output file path may have been created
    # as a directory instead of a CSV file path.
    if output_file.exists() and output_file.is_dir():
        if any(output_file.iterdir()):
            raise IsADirectoryError(
                f"Output path is a non-empty directory, not a file: {output_file}"
            )
        output_file.rmdir()
